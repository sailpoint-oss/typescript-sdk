...
**accountSelection** | **(optional)** `Array<SourceItemRef>` | The accounts where the access item will be provisioned to * Includes selections performed by the user in the event of multiple accounts existing on the same source * Also includes details for sources where user only has one account  | [default to undefined]
